...
namespace
{
  ...
  const std::wstring CREDENTIAL_FILTER = ToWide(ELEMENT_PREFERENCES_KEY_PREFIX + '*');
  ...
  PBYTE FlutterSecureStorageWindowsPlugin::GetEncryptionKey()
  {
      ...
      PBYTE AesKey;
      PCREDENTIALW pcred;
      const std::wstring target_name = ToWide("key_" + ELEMENT_PREFERENCES_KEY_PREFIX);
  ...
  std::optional<std::string> FlutterSecureStorageWindowsPlugin::Read(const std::string &key)
  {
      ...
      if (!fs.good()) {
          //Backwards comp.
          PCREDENTIALW pcred;
          const std::wstring target_name = ToWide(key);
          bool ok = CredReadW(target_name.c_str(), CRED_TYPE_GENERIC, 0, &pcred);
          if (ok)
          {
