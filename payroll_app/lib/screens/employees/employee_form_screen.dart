...
class _EmployeeFormScreenState extends State<EmployeeFormScreen> {
  ...
  double _dailyChildAllowance = 166667;
  ...
  // ضریب افزایش 1405
  double _selectedRate = AppConstants.salaryRateA; // 1.45 پیش‌فرض

  @override
  void initState() {
    super.initState();
    final e = widget.employee;
    _personnelCodeCtrl = TextEditingController(text: e?.personnelCode.toString() ?? '');
  ...
  void _autoCalculate1405() {
    ...
    // پایه سنوات روزانه = پایه ماهانه ÷ 30
    ...
    // حق تاهل
    if (_isMarried) {
      // اگر کارمند فرزند داشته باشد یا حداقل یک سال سابقه، دو برابر
      _dailyMarriage = _childrenCount > 0 || _hasPriorExperience
          ? 1108370
          : 554185;
    } else {
      _dailyMarriage = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.employee == null ? 'افزودن کارمند جدید' : 'ویرایش کارمند'),
        actions: [
          if (_saving)
            const Center(
              child: Padding(
                      ...
                      _row(children: [
                        Expanded(
                          child: SwitchListTile(
                            ...
                            secondary: Icon(
                              _isMarried ? Icons.favorite : Icons.favorite_border,
                              color: Colors.pink,
                            ),
                          ),
                        ),
                        ...
                        Expanded(
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                const Icon(Icons.child_care, color: Colors.lightBlue),
                      ...
                      _row(children: [
                        ...
                        Expanded(
                          child: PersianNumberField(
                            ...
                            onChanged: (v) => _dailyChildAllowance = v?.toDouble() ?? 0,
                          ),
                        ),
                      ]),
                      const SizedBox(height: 12),
                      _row(children: [
                        ...
                        Expanded(
                          child: PersianNumberField(
                            ...
                            onChanged: (v) => _lastYearSeniority = v?.toDouble() ?? 0,
                          ),
                        ),
                      ]),
                      const SizedBox(height: 12),
                      _row(children: [
                        Expanded(
                          child: PersianNumberField(
                            ...
                            onChanged: (v) => _otherBenefitsDaily = v?.toDouble() ?? 0,
                          ),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
